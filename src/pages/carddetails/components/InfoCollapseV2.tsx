...
import VideosDisplayV2 from "./VideosDisplayV2";
import ImagesDisplayV2 from "./ImagesDisplayV2";
import AudiosDisplayV2 from "./AudiosDisplayV2";
...
const InfoCollapseV2 = ({ data, evidences }: CardProps) => {
  ...
  const { card } = data;
  const cardStatus = getCardStatusAndText(card.status, card.cardDueDate, card.cardDefinitiveSolutionDate, card.cardCreationDate);
...
<div className="grid grid-rows-5 gap-y-4 gap-x-8 sm:grid-rows-none sm:gap-4 sm:px-4">
  ...
  <Row gutter={15}>
    ...
    <div className="flex items-center gap-1">
      <span className="font-semibold text-sm md:text-base">{Strings.tagNumber}</span>
      <p className="text-sm md:text-base">{card.siteCardId || Strings.NA}</p>
    </div>
      ...
      <Col span={8}>
    <div className="flex items-center gap-1">
      <span className="font-semibold text-sm md:text-base">{Strings.cardType}</span>
      <p
      ...
      <Col span={3}>
    <div className="flex items-center gap-1">
      <span className="font-semibold text-sm md:text-base">{Strings.status}</span>
      
      <span className="font-semibold text-sm md:text-base rounded-lg py-1 px-2 text-white cursor-pointer hover:bg-gray-600" 
      style={{ backgroundColor: primaryColor}}> {cardStatus.text} </span>
    ...
    <div className="flex items-center gap-1">
      <p className="font-semibold text-sm md:text-base">{Strings.tagPriority}</p>
      <p
...
<Row gutter={15}>
  ...
  <span className="font-semibold text-sm md:text-base">{Strings.creationDate}</span>
  <p className="text-left px-1 text-sm md:text-base">
    {formatDate(card.createdAt) || Strings.NA}
      ...
      <span className="font-semibold text-sm md:text-base">{Strings.dueDate}</span>
      <p className="text-left px-1 text-sm md:text-base">
      ...
      <div className="flex flex-col">
      <span className="font-semibold text-sm md:text-base">{Strings.dateStatus}</span>
      <p
        className={`text-white text-center font-bold px-2 py-1 rounded-md ${
          cardStatus.dateStatus === "Expired"
            ? "bg-red-500"
            : "bg-green-500"
        }`}
      >
      ...
      <div className="flex flex-col">
      <span className="font-semibold text-sm md:text-base">{Strings.daysSinceCreation}</span>
...
<Row gutter={15}>
      ...
      <div className="flex flex-col">
      <span className="font-semibold text-sm md:text-base">{Strings.problemType}</span>
      <p className="text-left px-1 text-sm md:text-base">
    ...
    <div className="flex flex-col">
      <span className="font-semibold text-sm md:text-base">{Strings.location}</span>
      <p className="text-left px-1 text-sm md:text-base">
...
<Row gutter={15}>
    ...
    <div className="flex items-center gap-1">
      <p className="font-semibold text-sm md:text-base">{Strings.tagMechanic}</p>
      <p
    ...
    {/* Anomalía detectada */}
    <div className="flex items-center gap-1">
      <p className="font-semibold text-sm md:text-base">{Strings.anomalyDetected}</p>
      <p className="text-sm md:text-base">{card.commentsAtCardCreation || Strings.NA}</p>
    </div>
      </Col>
  ...
  <div className="flex flex-wrap items-center gap-4 md:gap-8">
    <div className="flex items-center gap-1">
      <p className="font-semibold text-sm md:text-base">{Strings.createdBy}</p>
      <p className="text-sm md:text-base">{card.creatorName || Strings.NA}</p>
    </div>
  </div>

  </div>

  <Divider orientation="left" style={{ borderColor: '	#808080' }} className="text-sm md:text-base">Evidences at creation</Divider> 
